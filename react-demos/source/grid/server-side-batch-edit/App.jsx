		...
		return (
			<div>
			    <div className="demo-description">
			         <h1>Server-Side Batch Edit</h1>This example performs CRUD operations via Batch Editing. In order to enable
			        batch editing, the 'editing.batch' property should be set to 'true'. The
			        'editing.commandBar' setting allows you to display a tool bar or status
			        bar with tools for saving and reverting the edit made by the users. This
			        is achieved by setting 'editing.commandBar.visible' to 'true'.
			        <br/>
			        <br/>When the 'Save' button is clicked, the Grid sends server requests. The
			        server uses the SQL query sent by the Server-Side Datasource to get the
			        rows. The type of query is stored in the request's action property. Double-click
			        on a cell to begin edit.
			        <br />
			        ...
			        used.</div>
			    <Grid appearance={this.appearance} selection={this.selection} editing={this.editing} sorting={this.sorting} dataSource={this.dataSource} columns={this.columns} ref="grid" id="grid"></Grid>
			    <br />
			    <br />
			    	<h3>SQL Query</h3>
			    <div id="query"></div>
