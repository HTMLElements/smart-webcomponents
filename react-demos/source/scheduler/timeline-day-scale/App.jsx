...
class App extends React.Component {
	...
	dataSource = ['hour', 'halfHour', 'quarterHour', 'tenMinutes', 'fiveMinutes'];

	handleChange(event) {
		this.scheduler.current.timelineDayScale = event.detail.value;
	}
	...
	render() {
		return (
			<div>
				<Scheduler ref={this.scheduler} id="scheduler" dataSource={this.data} views={this.views}></Scheduler>
				<div className="options">
					<div className="description">
						<h4>Description</h4>
						<b>timelineDayScale</b> determines the scale of a day cell. By default
			            a day cell represents one day, but it can be splitted into half am hour,
			            quarter hour, ten minutes or five minutes.</div>
					<div className="option">
						<h4>Select Timeline Day Scale:</h4>
						<Input id="input" readonly dropDownButtonPosition="right"
							value="hour" dataSource={this.dataSource} onChange={this.handleChange.bind(this)}></Input>
					</div>
				</div>
			</div>
		);
