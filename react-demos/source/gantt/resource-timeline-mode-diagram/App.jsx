...
class App extends React.Component {
	...
	timelineHeaderFormatFunction = function (date, type, isHeaderDetailsContainer) {
		...
		if (isHeaderDetailsContainer) {
			return `Week ${this.ISO8601_week_no(date)}, ${date.toLocaleDateString(gantt.locale, { year: '2-digit' })}`;
		}
		else {
		...
		return (
			<div>
				<p> <b>Description:</b>The resource Timeline displays the working time of
			        ...
			        the working hours of a person on a task.</p>
				...
				( inside the TaskTree) that are assigned to that resource. The same goes
			        for the tasks inside the Task tree.</p>
				<GanttChart ref={this.ganttchart} id="ganttChart" dataSource={this.dataSource} taskColumns={this.taskColumns}
					resourceColumns={this.resourceColumns} treeMin={this.treeMin} resourcePanelSize={this.resourcePanelSize}
					view={this.view} nonworkingDays={this.nonworkingDays} timelineHeaderFormatFunction={this.timelineHeaderFormatFunction.bind(this)}></GanttChart>
			</div>
		);
	}
}
