...
class App extends React.Component {
	...
	cancelHandler(event) {
		const target = event.target,
			modal = this.window2.current,
			formWindow = this.window.current;

		target.closest('smart-window') === modal.nativeElement ? modal.close() : formWindow.close();
	}
	...
	handleReady(type) {
		const modal = this.window2.current,
			[labelA, labelB] = type === 'modal' ? ['No', 'Yes'] : ['Cancel', 'Agree'];

		ReactDOM.render(<section>
			<Button className="cancel" onClick={this.cancelHandler.bind(this)}>{labelA}</Button>
			<Button className="agree" onClick={this.agreeHandler.bind(this)}>{labelB}</Button>
	...
	render() {
		return (
			<div>
				...
				<Window ref={this.window} id="formWindow" opened label="Window" onReady={this.handleReady.bind(this)}>
					<div id="article">
						<section>
						...
						<section>
							<h3>More</h3>
							<p>Fusce blandit eros vitae libero iaculis, sit amet rhoncus augue dignissim.
							...
							Donec et massa nisl. Etiam dignissim accumsan eleifend. Aliquam rhoncus
			                    lorem sed purus congue, vitae eleifend tellus scelerisque.</p>
						</section>
					</div>
				</Window>
				<Window ref={this.window2} id="modal" modal windowParent="formWindow" label="Dialog"
					headerButtons={["close"]} onReady={this.handleReady.bind(this, 'modal')}>
					<h3>Are you sure ?</h3>
				</Window>
			</div>
		);
