...
class App extends React.Component {
	...
	init() {
		...
		options.addEventListener('change', function (event) {
			...
			if (target instanceof window.Smart.SwitchButton) {
				editor.toolbarSticky = event.detail.value;
			}
		});
	};
	...
	render() {
		return (
			<div>
				<Editor ref={this.editor} id="editor" toolbarItems={this.toolbarItems} toolbarSticky={this.toolbarSticky}>
					<div>
				...
				<div ref={this.options} className="options">
					<div className="description">
						<h4>Enable sticky Toolbar</h4>
						<p>When enabled the Toolbar will stick to the Top of the window while scrolling
			                the page.</p>
					</div>
					<div className="option">
						<SwitchButton ref="switchbutton" checked></SwitchButton>
					</div>
				</div>
			</div>
		);
