...
class App extends React.Component {
	...
	createSparkline(element, index, app) {
		let type;
		if (index % 2 === 0) {
			type = 'column';
		} else if (index === 3) {
		...
		const settings = {
			...
			dataSource: app.sampleData[index].dailyTrend,
			xAxis: {
				visible: false,
				valuesOnTicks: false
			},
			seriesGroups: [{
				...
				series: [{
					...
					colorFunction: (value, itemIndex, serie, group) => {
						return (value < 10) ? app.negativeColor : app.positiveColor;
					}
				}]
			}]
		};
	...
	init() {
		...
		that.sampleData.forEach(function (dataPoint, index) {
			...
			for (let field in dataPoint) {
				...
				if (field !== 'dailyTrend') {
					...
					ReactDOM.render(<Chart />, td, function () {
						that.createSparkline(this, index, that);
					});
				}
				tr.appendChild(td);
			}
