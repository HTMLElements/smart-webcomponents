...
function initListBox() {
    ...
    for (let i = 0; i < listbox.dataSource.length; i++) {
        items[i].querySelector('img').src = `../../images/people/${listbox.dataSource[i].image}.png`;
    }
}
...
function updatePersonData() {
    ...
    const items = listbox.items;
    items[selectedPersonId].querySelector('img').src = `../../images/people/${data[selectedPersonId].image}.png`;
}
function resetForm() {
    const selectedPerson = data[selectedPersonId];
    ...
    document.querySelector('#personInfo img').src = `../../images/people/${selectedPerson.image}.png`;
