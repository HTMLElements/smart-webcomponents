window.initProjectsList1 = function () {
    const projectssData = [
        {
            name: 'Tanzanite',
    ...
    new window.Smart.CardView('#cardView', {
        dataSource: new Smart.DataAdapter(
            {
                dataSource: projectssData,
                dataFields:
                    [
                        'name: string',
                        'images: string',
            ...
            { label: 'Images', dataField: 'images', image: true },
            {
                label: 'assignees', dataField: 'assignees', formatFunction: function (settings) {
                    ...
                    photoList.forEach(photoName => {
                        htmlResult += `<div class="w-30px square rounded-circle mr-2" style="background-image: url('../../images/phonebook/${photoName}.png'); background-size: cover;"></div>`;
                    });
