...
function updateColorScheme(colorScheme) {
    ...
    const lastItemofPath = window.location.pathname.substring(window.location.pathname.lastIndexOf('/') + 1)
    let fnName = Smart.Utilities.Core.toCamelCase('init-' + lastItemofPath.replace('/', ''));
    fnName = fnName.replace(/-/g, '');
    fnName = fnName + 'ChartComponents';
    // console.log(fnName);
    if (window[fnName]) {
        window[fnName]();
    }
...
function changeMainCssFile(themeColor) {
    const theme = document.body.getAttribute('theme');

    let themeColorThemeFile = themeColor;

    if (themeColor != '') {
        themeColorThemeFile = '_' + themeColor;
    }

    let themeCSS = './assets/css/' + theme + '/main' + themeColorThemeFile + '.css';

    document.getElementById("main-css").href = themeCSS;
