...
class YearQuarters {
  ...
  constructor() {
    this.Q1 = ['January', 'February', 'March']
    this.Q2 = ['April', 'May', 'June']
    this.Q3 = ['July', 'August', 'September']
    this.Q4 = ['October', 'November', 'December']
  }
}
...
export class DataService {
  ...
  getEmployees() {
    ...
    const data: Employee[] = new Array();

    const firstNames = ["Nancy", "Andrew", "Johnny", "Maya", "Steven", "Mark", "Monica", "Laura", "Anne"];
    ...
    for (let i = 0; i < firstNames.length; i++) {
      const row: Employee = {
        firstName: firstNames[k],
        lastName: lastNames[k],
        title: titles[k],
        titleOfCourtesy: titleofcourtesy[k],
        birthDate: birthdate[k],
        hireDate: hiredate[k],
        address: address[k],
        city: city[k],
        postalCode: postalcode[k],
        country: country[k],
        homePhone: homephone[k],
        notes: notes[k],
      };
  ...
  generateData(rowscount: number, hasNullValues?: boolean) {
    // prepare the data
    ...
    for (let i = 0; i < rowscount; i++) {
      ...
      const row: DataEntity = {
        id: i,
        reportsto: Math.floor(Math.random() * firstNames.length),
  ...
  generateOrdersData(rowscount: number) {
    // prepare the data
    interface Order {
      id: string | number,
    ...
    for (let i = 0; i < rowscount; i++) {
      ...
      const productindex = Math.floor(Math.random() * productNames.length);
      const price = parseFloat(priceValues[productindex]);
      const quantity = 2 + Math.round(Math.random() * 10);
      ...
      const row: Order = {
        ...
        firstname: firstNames[Math.floor(Math.random() * firstNames.length)],
        lastname: lastNames[Math.floor(Math.random() * lastNames.length)],
        customer: companyNames[Math.floor(Math.random() * companyNames.length)],
        date: date,
        price: 0,
        quantity: 0
      };
  ...
  generateCarsData() {
    ...
    const fuelType = ["Any", "Diesel", "Electric", "Ethanol (FFV, E85, etc.)", "Gas", "LPG", "Natural Gas", "Hybrid", "Hydrogen", "Petrol"];
    const vehicleType = ["Saloon", "Small Car", "Estate Car", "Van / Minibus", "Off-road Vehicle/Pickup Truck", "Cabriolet / Roadster", "Sports Car/Coupe"];
    const power =
      [
    ...
    for (let i = 0; i < makes.length; i++) {
      ...
      const row: Car = {
        ...
        vehicleType: vehicleType[Math.floor(Math.random() * vehicleType.length)],
        powerFrom: power[powerIndex],
        powerTo: power[powerIndex + 1],
  ...
  getCountriesCodesData() {
    const data = [
      ...
      ["Vanuatu", "VU", "VUT", 548],
      ["Venezuela (Boliconstian Republic of)", "VE", "VEN", 862],
      ["Viet Nam", "VN", "VNM", 704],
  ...
  getCountriesCurrencyData() {
    const data = [
      ...
      ["VENEZUELA (BOLIconstIAN REPUBLIC OF)", "Boliconst", "VEF", 937],
  ...
  getKanbanData(locale = 'en') {
    ...
    const data: DataEntity[] = [
      ...
      comments = [
        ...
        'Aenean ultrices maximus ex id scelerisque. Suspendisse cursus maximus nulla, sed ornare lectus aliquet eu. Orci constius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.',
  ...
  getKanbanHierarchicalData() {
    ...
    return [data, ...this.getKanbanData()];
  }

  generatePivotData(numberOfRecords: number, numberOfYears = 4) {
    const continents = ['Africa', 'Asia', 'Australia', 'Europe', 'North America', 'South America'];

    const cities: ContinentCities = {
    ...
    const months = new YearQuarters();

    const maxYear = new Date().getFullYear();
    const minYear = maxYear - numberOfYears + 1;
    ...
    for (let i = 0; i < numberOfRecords; i++) {
      ...
      dataPoint.city = cities[dataPoint.continent as keyof ContinentCities][Math.floor(Math.random() * cities[dataPoint.continent as keyof ContinentCities].length)];
      dataPoint.year = Math.floor(Math.random() * (maxYear - minYear + 1)) + minYear;
      dataPoint.quarter = ['Q1', 'Q2', 'Q3', 'Q4'][Math.floor(Math.random() * 4)];
      dataPoint.month = months[dataPoint.quarter as keyof YearQuarters][Math.floor(Math.random() * 3)];
    ...
    if (Smart.DataAdapter) {
      result = new Smart.DataAdapter({
        dataSource: result,
        dataFields: [
          'continent: string',
          'city: string',
      ...
      result._sort(result.boundSource, ['year', 'quarter', 'month'], ['asc', 'asc', 'asc'], ['number', 'string', 'string'],
        function (dataSource: any, sortColumns: string[], directions: string[], compareFunctions: any) {
  ...
  generateGanttChartDataTree(count = 50, minDate: Date, maxDate: Date) {
    const data = [];
    ...
    const getTasks = (n = 1, includeSubProjects?: boolean): any => {
      let tasks = [];
      ...
      for (let i = 0; i < n; i += 1) {
        ...
        dateMax.setDate(dateMax.getDate() + rand + this.getRandom());
        taskCounter++;
      }
  ...
  generateGanttChartDataFlat(count = 50, minDate: Date, maxDate: Date) {
    const data = [];
    ...
    if (!maxDate || isNaN(new Date(maxDate).getTime())) {
      maxDate = new Date(minDate.getFullYear(), minDate.getMonth(), minDate.getDate() + this.getRandom(50));
    }
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      dateMinMonth = rand;
      dateMaxMonth = rand + this.getRandom(20);
      dateMinDate = this.getRandom();
      dateMaxDate = this.getRandom(20);
      taskCounter++;
