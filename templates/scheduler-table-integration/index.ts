...
import { Scheduler, Table, ListBox, Input } from "../../source/typescript/jqx.elements"
...
function handleItemTemplate(item: HTMLElement, settings: { label: string, value: string }) {
...
function dateFormatFunction(data: { cell: HTMLElement, value: any }) {
    const scheduler = document.querySelector('jqx-scheduler')!;
        ...
        return {
            ...
            columns: [
                ...
                { label: 'Event', dataField: 'label', dataType: 'string' },
                {
                    ...
                    editor: {
                        template: '<jqx-date-time-picker calendar-button drop-down-append-to="body"></jqx-date-time-picker>',
                        onRender(row: any, column: string, editor: Input, value: string) {
                            editor.value = value;
                        },
                        getValue(editor: Input) {
                            return editor.value
                        }
                    }
                },
                ...
                { label: 'Description', dataField: 'description', dataType: 'string' },
                {
                    label: 'Status', dataField: 'status', dataType: 'string', editor: {
                        template: '<jqx-input drop-down-button-position="right" readonly></jqx-input>',
                        onInit(row: any, column: string, editor: Input, value: string) {
                            editor.dataSource = document.querySelector('jqx-scheduler')!.statuses;
                        },
                        onRender(row: any, column: string, editor: Input, value: string) {
                            editor.value = value;
                        },
                ...
                },
                {
                    ...
                    formatFunction(settings: { template: string, value: number }) {
                        const value = settings.value,
                            resourceItem = resoruceItems.find(i => i.id === value);
                    ...
                    editor: {
                        template: '<jqx-drop-down-list drop-down-append-to="body"></jqx-drop-down-list>',
                        onInit(row: any, column: string, editor: Input, value: string) {
                            editor.itemTemplate = handleItemTemplate;
                            editor.dataSource = resoruceItems;
                        },
                        onRender(row: any, column: string, editor: Input, value: number) {
                ...
                },
                {
                    ...
                    formatFunction(settings: { template: string, value: number }) {
                        settings.template = '<jqx-button class="delete-button"><i class="material-icons">&#xE5CD;</i></jqx-button>';
                    },
                },
            ]
        };
...
window.onload = function () {
    const scheduler = document.getElementById('scheduler')! as Scheduler,
        listBox = document.getElementById('listBox')! as ListBox,
        table = document.getElementById('table')! as Table;
    let filteredEvents: object[] = [];

    function refreshTableData(event: any) {
        ...
        if (event.type === 'itemInsert') {
            ...
            const maxId = scheduler.events!.reduce((acc: number, e: any) => acc = Math.max(parseInt(e.id || 0), acc), 0);
    ...
    function refreshSchedulerData(event: any) {
    ...
    function refreshData(event: any) {
        ...
        for (let i = 0; i < events.length; i++) {
            const eventObj = events[i] as any;
    ...
    function removeEvent(event: any) {
        ...
        if (detail.originalEvent.target.closest('.jqx-button.delete-button')) {
