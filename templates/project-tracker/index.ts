...
import { Grid, Tabs } from "../../source/typescript/smart.elements";

declare var smartDataAdapter: any;

window.onload = function () {
    const grid1 = <Grid>document.getElementById('tab1Grid');
    const grid2 = <Grid>document.getElementById('tab2Grid');
    const grid3 = <Grid>document.getElementById('tab3Grid');
    const tabs = <Tabs>document.getElementById('tabs');

    const tabStrip = document.querySelector('.smart-tab-strip')!;
    const recordsSpan = <HTMLSpanElement>document.createElement('span');
...
function onRowDetailInit(index: number, row: any, detail: any) {
    ...
    (row.grid as Grid).rowDetail!.dialog!.header = row.data.name;
}