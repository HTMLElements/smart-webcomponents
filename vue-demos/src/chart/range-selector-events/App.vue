<template>
  <div class="vue-root">
    ...
    <br />
    <textarea id="eventsTextArea"></textarea>
  </div>
</template>
  ...
  setup() {
    onMounted(() => {
      const sampleData = [
        ...
        toolTipCustomFormatFn = function(
          value,
          itemIndex,
          serie,
          group,
...
<b>Date:</b> ${xAxisValue.getDate()}-${monthFormatter.format(
            xAxisValue
          )}-${xAxisValue.getFullYear()}</b>
<br />
...
<b>Daily volume:</b> ${volume}
      ...
      const chart = document.getElementById("chart"),
        eventsTextArea = document.getElementById("eventsTextArea");
      ...
      function dumpEventInfo(event) {
        ...
        if (
          event.type.indexOf("refresh") !== -1 &&
          chart !== eventDetail.instance
        ) {
          return;
        }
        const date = new Date();
        let text = eventsTextArea.value,
          line =
            "Time: " +
            (date.getHours() +
              ":" +
        ...
        if (event.type === "toggle") {
          if (chart.seriesGroups) {
            line +=
              ", series group index: " +
              chart.seriesGroups.indexOf(eventDetail.seriesGroup);
        ...
        if (event.type.indexOf("rangeSelection") !== -1) {
          line +=
            ", minValue: " +
            eventDetail.minValue.getFullYear() +
            "-" +
            (eventDetail.minValue.getMonth() + 1) +
            "-" +
            eventDetail.minValue.getDate();
          line +=
            ", maxValue: " +
            eventDetail.maxValue.getFullYear() +
            "-" +
            (eventDetail.maxValue.getMonth() + 1) +
            "-" +
            eventDetail.maxValue.getDate();
        }
        text = line + "\n" + text;
        eventsTextArea.value = text;
      }
      [
        "click",
        "mouseover",
        ...
        "refreshBegin",
        "refreshEnd"
      ].forEach(eventType => chart.addEventListener(eventType, dumpEventInfo));
    });
  }
};
</script>
