<template>
  <div class="vue-root">
    <div class="demo-description">This example shows you how to auto-generate a Form from its value.</div>
    <div id="employeeSelect">
      <h3>Select Employee</h3>
      <smart-drop-down-list id="employeeList"></smart-drop-down-list>
    </div>
    <br />
    ...
    onMounted(() => {
      const employees = (() => {
        ...
        for (let i = 0; i < firstNames.length; i++) {
          const row = {
            Photo: `../../../images/phonebook/${firstNames[
              k
            ].toLowerCase()}.png`,
            "First Name": firstNames[k],
            "Last Name": lastNames[k],
            Title: titles[k],
            "Title of Courtesy": titleofcourtesy[k],
            "Birth Date": birthdate[k],
            "Hire Date": hiredate[k],
            Address: address[k],
            City: city[k],
            "Postal Code": postalcode[k],
            Country: country[k],
            "Home Phone": homephone[k],
            Notes: notes[k]
          };
          data[i] = row;
          k++;
      ...
      const form = new window.Smart.Form("#employeeForm", {
        value: employees[0]
      });
      const employeeList = document.querySelector("#employeeList");
      //const avatar = document.querySelector("#avatar");
      employeeList.dataSource = employees.map(value => {
        return value["First Name"] + " " + value["Last Name"];
      });
      employeeList.selectedIndexes = [0];
      employeeList.onchange = () => {
        const employeeIndex = employeeList.selectedIndexes[0];
        form.value = employees[employeeIndex];
      };
    });
  }
};
