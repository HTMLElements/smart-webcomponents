<template>
  <div class="vue-root">
    <smart-carousel keyboard slide-show auto-play loop></smart-carousel>
  </div>
</template>
    ...
    onMounted(() => {
      ...
      function generateDataSource(items) {
        ...
        for (let i = 0; i < items; i++) {
          ...
          const item = {
            image: `${basePath}carousel-large-${i + 1}.jpg`,
            label: headlines[labelIndex],
            content:
              contentParagraphs[paragraphIndex].substring(0, subParagraph) + "."
          };
