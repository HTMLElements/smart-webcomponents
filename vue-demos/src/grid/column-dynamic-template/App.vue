<template>
  <div class="vue-root">
    <div class="demo-description">
      Columns 'template' property is set to a custom function in order to render
...
1297,December,Company C,Northeast,New Jersey,Atlantic City,Marcus Kowalski,Credit Card,Fruit Preserves,Marmelade,4.5,300,1350,21.04`;
      ...
      function getColor(value, field) {
        ...
        const colorScheme = ["#F82B60", "#FCB400", "#20C933"];
        const minValue = 0;
              ...
              columns: [
                {
                  ...
                  template: function(formatObject) {
                    if (!formatObject.template) {
                      const salesValue =
                        '<span style="padding: 4px 10px; border-radius: 10px; color: #fff; background: ' +
                        getChipColor() +
                        '";>' +
                        formatObject.row.data["Salesperson"] +
                        "</span>";
                      const categoryValue =
                        '<span style="margin-left: 10px; padding: 4px 10px; border-radius: 10px; color: #fff; background: ' +
                        getChipColor() +
                        '";>' +
                        formatObject.row.data["Category"] +
                        "</span>";
                      const productValue =
                        '<span style="margin-left: 10px; padding: 4px 10px; border-radius: 10px; color: #fff; background: ' +
                        getChipColor() +
                      ...
                      salesSpan.style.background = getChipColor();
                      salesSpan.innerHTML = "" + salesValue;
                      categorySpan.style.background = getChipColor();
                      categorySpan.innerHTML = "" + categoryValue;
                      productSpan.style.background = getChipColor();
                ...
                },
                {
                  ...
                  dataField: "Quantity",
                  template: function(formatObject) {
                    const color = getColor(formatObject.value, "Quantity");
                    if (!formatObject.template) {
                      formatObject.template =
                        '<span style="margin-right: 7px; padding: 4px 18px; border-radius: 10px 5px; color: #fff; background: ' +
                        color +
                        '";>' +
                        formatObject.value +
                        "</span>";
