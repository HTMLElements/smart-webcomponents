...
export class AppComponent implements AfterViewInit, OnInit {	 
    @ViewChild('employeeList', { read: DropDownListComponent, static: false }) employeeList!: DropDownListComponent;
	...
	ngAfterViewInit(): void {
		...
		const form = new Smart.Form('#employeeForm',
		{	
			value: employees[0]
		});

		this.employeeList.dataSource = employees.map((value) => {
			return value['First Name'] + ' ' + value['Last Name'];
		});

		this.employees = employees;		
		this.employeeList.selectedIndexes = [0];
		this.form = form;
    }
	
	onChange($event) {
		if (this.employees) {
			const employeeIndex = this.employeeList.selectedIndexes[0];
