...
import { AccordionComponent, Smart, Accordion, AccordionItem } from 'smart-webcomponents-angular/accordion';
...
export class AppComponent implements AfterViewInit, OnInit {
    ...
    @ViewChild('rowSeparator', { read: ElementRef, static: false }) rowSeparator: ElementRef;
    ...
    init(): void {
        ...
        const that = this;
        let tripTypeRadios = [that.radiobutton, that.radiobutton2];

        const loadAccordion = (e: CustomEvent): void => {
            if (e.detail.content) {
                return;
            }

            const item: AccordionItem = (<Accordion>e.target).getElementsByTagName('smart-accordion-item')[e.detail.index] as AccordionItem;

            requestAnimationFrame(() => {
                const accordionItem: AccordionItem = document.getElementById(item.id.replace("Item", "")) as AccordionItem;

                (<HTMLElement>item.content).appendChild(accordionItem.content as HTMLElement);
            });
        };
        ...
        const loadGridInfo = () => {
            if (document.getElementById('infoGrid') instanceof window.Smart.Grid) {
                return;
            }
            new window.Smart.Grid('#infoGrid', {
                sorting: {
                    enabled: true
                },
                layout: {
        ...
        const returnCalendarContainer = that.rowSeparator.nativeElement;

        for (let i = 0; i < tripTypeRadios.length; i++) {
            tripTypeRadios[i].addEventListener('change', function (e: CustomEvent): void {
                if (((<HTMLElement>e.target).classList.contains('return')) && (returnCalendarContainer.classList.contains('smart-hidden'))) {
        ...
        that.tabs.addEventListener('change', loadGrid);

        that.accordion.addEventListener('expand', loadAccordion);
