...
export class AppComponent implements AfterViewInit, OnInit {
    ...
    init(): void {
        ...
        this.carousel.dataSource = generateDataSource(6, 30, 250);

        function generateDataSource(items = 1, headlineLength = 10, contentLength = 100) {
            ...
            for (let i = 0; i < items; i++) {
                const image = `${basePath}carousel-medium-${i + 1}.jpg`, headline = headlines[parseInt('' + Math.random() * 5)].substring(0, headlineLength), content = contentParagraphs[parseInt('' + Math.random() * 5)].substring(0, contentLength) + '.';
                let item = {
                    image: image,
                    headline: headline,
                    content: content
