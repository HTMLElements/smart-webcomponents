...
export class AppComponent implements AfterViewInit, OnInit {
    ...
    valueAxis = {
        ...
        formatSettings: { decimalPlaces: 0, sufix: '°C' }
    };
    seriesGroups = [
        {
            type: 'splinerangearea',
            series: [
                { dataFieldFrom: 'Low', dataFieldTo: 'High', displayText: 'Temperature Range', opacity: 0.8, toolTipFormatSettings: { decimalPlaces: 0, sufix: '°C' } }
            ]
        },
        {
            type: 'spline',
            series: [
                { dataField: 'Average', symbolType: 'circle', symbolSize: 9, toolTipFormatSettings: { decimalPlaces: 0, sufix: '°C' } }
            ]
        }
    ];
    ...
    init(): void {
        // init code.
        const sampleData: { Date: Date, Low: number, High: number, Average!: number }[] = [
        ...
        sampleData.forEach(dataPoint => dataPoint.Average = (dataPoint.High + dataPoint.Low) / 2);
